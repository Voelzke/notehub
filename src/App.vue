    ...
    methods: {
        ...
        async openNote(note) {
            ...
            try {
                ...
                // Restore from localStorage backup if newer
                if (this._pendingBackup && this._pendingBackup.id === note.id) {
                    const backup = this._pendingBackup
                    this._pendingBackup = null
                    if (backup.content && backup.content !== response.data.content) {
        ...
        closeMobileSidebar() {
            if (window.innerWidth < 1024) {
                ...
                if (nav) {
                    nav.classList.add('app-navigation--close')
                }
            }
        },
        ...
        async saveNote() {
            ...
            try {
                ...
                if (this.saveRetryCount < 3) {
                    this.saveRetryCount++
                    const delay = Math.min(1000 * Math.pow(2, this.saveRetryCount), 8000)
                    console.warn('Retrying save in ' + delay + 'ms (attempt ' + this.saveRetryCount + ')')
                    this.saving = false
        ...
        // Save to localStorage on beforeunload (real save happens on next open)
        saveNoteSync() {
            if (!this.currentNote || this.currentNoteReadonly) return
            this.saveLocalBackup()
        },
        ...
        saveLocalBackup() {
            if (!this.currentNote) return
            try {
                localStorage.setItem('notehub-backup-id', String(this.currentNote.id))
                localStorage.setItem('notehub-backup-title', this.currentNote.title || '')
                localStorage.setItem('notehub-backup-content', this.currentNote.content || '')
                localStorage.setItem('notehub-backup-time', String(Date.now()))
            } catch (e) {
        ...
        clearLocalBackup() {
            try {
                localStorage.removeItem('notehub-backup-id')
                localStorage.removeItem('notehub-backup-title')
                localStorage.removeItem('notehub-backup-content')
                localStorage.removeItem('notehub-backup-time')
            } catch (e) {
                // ignore
            }
        },
        ...
        restoreLocalBackup() {
            try {
                const backupId = localStorage.getItem('notehub-backup-id')
                const backupTime = localStorage.getItem('notehub-backup-time')
                if (!backupId || !backupTime) return
                // Only restore if backup is less than 1 hour old
                if (Date.now() - Number(backupTime) > 3600000) {
                    this.clearLocalBackup()
                    return
                }
                const title = localStorage.getItem('notehub-backup-title')
                const content = localStorage.getItem('notehub-backup-content')
                if (title || content) {
                    console.info('NoteHub: Found unsaved backup for note ' + backupId + ', will restore on open')
                    this._pendingBackup = { id: Number(backupId), title, content }
                }
            } catch (e) {
                // ignore
            }
